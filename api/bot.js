...
function endDialogMarkup(orderNumber) {
  return {
    inline_keyboard: [
      [
        {
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (action === "contact") {
        ...
        await sendMessage(
          adminId,
          `Открыт диалог по заказу #${orderNumber}.\nСледующее ваше сообщение будет отправлено покупателю.\n\nЧтобы закончить, нажмите «Завершить диалог».`,
          {
            reply_markup: endDialogMarkup(orderNumber),
          }
        );

        await sendMessage(
          order.customerChatId,
          `💬 Администратор открыл диалог по вашему заказу #${orderNumber}.\nТеперь вы можете отвечать прямо сюда.`
        );
      ...
      if (action === "end") {
        ...
        const noActiveAdmins = !activeAdmins || activeAdmins.length === 0;
        ...
        if (noActiveAdmins) {
          await sendMessage(
            order.customerChatId,
            `✅ Диалог по вашему заказу #${orderNumber} завершён.\nЕсли администратор напишет снова, диалог снова откроется.`
          );
        }
    ...
    if (isAdmin(chatId)) {
      ...
      if (modeRaw && text) {
        ...
        await sendMessage(chatId, "Сообщение отправлено покупателю.", {
          reply_markup: endDialogMarkup(mode.orderNumber),
        });
      }
